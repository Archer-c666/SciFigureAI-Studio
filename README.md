...
- 图像预览区重新布局：
  - 工作区改为“上方预览 / 下方助手对话”分区
  - 预览图带状态提示与参数摘要
  - 画布放入可滚动容器中，显示更居中、更稳定
- 导入数据后不再自动绘图：
  - 系统仅分析数据并刷新默认方案
  - 需要用户点击“按参数绘制”或在助手区发出绘图指令后才真正绘制
- 右侧参数面板新增可选字段映射：
  - X 数据列（可选）
  - Y 数据列（可选）
  - Z 数据列（可选，三维图）
  - 保持“自动”时，由系统根据图表类型和数据结构自动匹配
- “选择特征 / 标签 / X-Y-Z / 样本”窗口说明更新：
  - X/Y/Z 都为可选项
  - 二维图通常只需 X/Y
  - 三维图才需要 Z
  - 保持自动即可后续自动匹配
- 更灵活的使用方式：
  - 二维散点图：可指定 X/Y，也可保持自动
  - 三维散点图：可指定 X/Y/Z，也可只提供部分字段让系统辅助选择
  - 非三维图无需填写 Z
